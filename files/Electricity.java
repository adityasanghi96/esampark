...
class Electricity extends JFrame implements ActionListener,ItemListener
{
	...
	Electricity(String t)
	{
		...
		head=new JLabel("Electricity Bill");
					...
					if(e.getKeyCode() == 10) 
					{
						...
						else
						{
							...
							try{
								...
								String x = "Select * from Electricity_bill where meter_no='"+meter+"' ";
			...
			else
			{
				...
				if(ei==2 && (sappr.equals("") || snum.equals("") || sbank.equals(""))){
					...
					try{
						...
						Connection con = DriverManager.getConnection("jdbc:mysql:///suvidha","root","");
						Statement st = con.createStatement();			
						String u="Update Electricity_bill set bill='"+remain+"' where meter_no='"+meter1+"' ";
						int i=st.executeUpdate(u);
						if(i>0)
						{
							String q="insert into Electricitysum(meter,amount,date,type,num,app,bank) values('"+meter1+"','"+t_billclear.getText()+"','"+strDate+"','"+typ+"','"+snum+"','"+sappr+"','"+sbank+"')";
